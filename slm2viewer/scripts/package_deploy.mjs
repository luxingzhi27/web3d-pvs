...
# Scene metadata and proxy geometry are served from this deployment.
# Each scene's loaderConfig controls its GLB base URL.
#
# Usage:
...
function writeManifest(copied, skipped, compressed, indexAssetVersions, obfuscated) {
  ...
  writeFileSync(resolve(deployDir, 'deploy_manifest.json'), JSON.stringify({
    ...
    deploymentAssets: {
      ...
      note: 'Scene metadata and proxy geometry are local; GLB paths come from each scene loaderConfig.',
    },
    indexAssetVersions,
    obfuscation: {
      enabled: obfuscateJs,
