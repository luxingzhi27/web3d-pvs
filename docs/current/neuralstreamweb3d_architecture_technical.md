...
实例化场景与 GLB 表面点
        ...
        124 维固定实例表 + 轻量查询权重
                         │
真实相机 ──> 66° 后退 AABB 候选 ──> 一次 Worker/WebGPU 批量查询
                         │
                  实例可见性概率
                         │
             60° 真实视锥实例过滤
...
Worker 执行 AABB 候选扫描、WebGPU 推理、真实视锥过滤和 GLB 聚合。主线程只接收最终实例编号和下载计划，不重复做第二份 AABB 过滤。
...
- AABB 候选扫描仍在 Worker CPU 上完成，WebGPU 负责神经查询。
