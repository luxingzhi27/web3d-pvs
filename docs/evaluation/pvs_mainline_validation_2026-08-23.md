...
- `evaluate_pvs.py`：诊断重校准显式记录 calibration 与 validation 的独立 bootstrap seed；
- `reaudit_pvs.py`：新增正式快照重审计、checkpoint 元数据冻结、逐 pose 回放和 paired bootstrap 入口；
- `run_pvs.py`：新正式训练固定 `10,000` 次 bootstrap，并拒绝把历史 `2,000` 次成员重新汇总为正式结果；
