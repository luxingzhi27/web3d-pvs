...
仅当负例分数与正例分数违反 `0.5` logit 间隔时，使用温度为 `0.25` 的 softplus 间隔损失。当前 `contrastive mix=0`，因此该项只分离最终 logit，不再通过投影头强制隐藏表示聚类。这样保留了困难边界优化的作用，同时避免表征对比损失对几何和关系特征施加过强的类内收缩。
...
| 变体 | 改动 | 保留内容 | 研究问题 |
|---|---|---|---|
| **Full：无对比损失** | 无 | 全部分层关系、生存场、矩包络和三项可见性损失 | 当前完整参考 |
...
所有变体的三个种子都通过了 validation weighted recall 点估计与单侧 95% 下界大于 `0.99` 的安全门，因此可在相同安全约束下比较分类和剔除效率。Full 三个种子的选中 epoch/阈值为 `32/0.42`、`36/0.68` 和 `40/0.58`。
...
旧的表征对比项把困难低分正例拉向正类原型，并推离高分负例。该目标假设可见正例在隐藏空间中应形成紧凑类簇，但实例可见性由几何、遮挡关系、观察方向和深度共同决定，同一“可见”标签可能对应完全不同的几何条件。强制类内聚类会压缩对最终边界有用的连续结构。

实验证实，旧含对比 Full 相对当前无对比 Full 的 aggregate precision 下降 `0.01554`，accuracy 下降 `0.00819`，balanced accuracy 下降 `0.00356`，useful cull 下降 `0.00822`，并多预测 `39.17` 个实例；这些区间均不跨零，weighted recall 差值跨零。因此当前模型只保留同一困难样本集合上的 logit 间隔，不再使用表征投影和对比聚类。
