...
正式输入中，MacBook Air M2 与 vivo X200 Pro mini 各有 HKUST WebGPU 的 `5 × 684` 次 pose 查询。A6000 自动化文件只有一轮，作为 smoke 排除；IFCBench 没有移动端五轮上传，相关设备/场景/后端行标为 `unavailable`，没有用 smoke 数值补齐。
