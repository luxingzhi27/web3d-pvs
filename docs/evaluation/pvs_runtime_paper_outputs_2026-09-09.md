...
正式输入中，MacBook Air M2 与 vivo X200 Pro mini 各有 HKUST WebGPU 的 `5 × 684` 次 pose 查询；RTX A6000 已完成 HKUST `5 × 684` 和 IFCBench `5 × 2710` 的 WebGPU/WASM 测量。IFCBench 移动端组合仍标为 `unavailable`，没有用桌面结果补齐。
