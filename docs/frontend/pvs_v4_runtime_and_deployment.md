...
本文是当前前端神经剔除的唯一运行说明，覆盖 V4 运行资产、Worker 查询、实例显示、GLB 调度和部署验证。
...
校准阈值为 `0.6800000071525574`，由 checkpoint 自己的 calibration split 冻结。该工作点的 aggregate weighted recall 为 `0.9978366`，单侧置信下界为 `0.9963827`。当前导出对应 seed `20260802` 的 Full checkpoint，最佳安全 checkpoint 位于 epoch 36；前端不得自行重选阈值。

运行时每个实例保存 `96` 维几何特征和 `28` 维已融合的逐实例生存场系数，共 `124` 维 FP16 固定特征。离线分层关系网络、逐实例校准、点云编码和遮挡证据生成均不在浏览器执行。默认计算后端为 `auto`：优先在 Worker 内运行 WebGPU；WebGPU 不存在、adapter/device 初始化失败或运行中设备失效时，自动在同一个 Worker 内切换到 WASM SIMD 后端继续运行同一模型。
...
远端请求失败后，只要该 GLB 仍属于当前下载计划，就会重新进入下载队列，最多重试三次；切换视点后已经离开计划的资源不重试。该规则同时适用于立即队列和预取队列。
...
当前浏览器功能 smoke 中，初始视点得到 `9398` 个候选、`3728` 个神经可见实例和 `3633` 个真实视锥显示实例；强制 WASM 和 WebGPU 初始化失败后的自动 WASM 输出一致，单次完整推理分别约为 `65.2 ms` 和 `67.2 ms`。该结果是服务器无头 Chrome 功能数据，不是移动端性能结论。
...
资源管线由事件推进：新计划、下载完成、解析完成、挂载完成和退避重试到期都会直接唤醒调度器。调度器按统一状态表继续处理待执行任务；当前真实视锥内仍失败的数量通过 `urgentFailed` 单独暴露，不能被算成预取或已完成。调试面板中的 `urgentMissing` 表示当前视锥目标减去已驻留数量，可能处于下载、解析、挂载或重试阶段。
...
当前神经后端输出的真实 `60°` 最终实例集合是神经模式下唯一的显示依据。屏幕面积只用于 LOD 或下载排序，不参与神经后端的实例显示判定。
...
场景不创建 Three.js Fog，配置文件和控制面板不提供雾化、远景雾区或动态分辨率选项。

## 按需渲染与低频维护

页面只在相机交互、键盘/触摸移动、自动旋转、轨迹采集、显式动画或资源/实例状态变化时请求新帧。控制面板折叠、相机静止且没有待挂载工作时停止主 RAF。

OrbitControls、键盘和触摸输入只在相机实际变化时登记一次待更新，达到既有 `80 ms` 合并间隔后执行最新相机的剔除。下载请求和 GLB 解析由异步回调推进；只有解析完成后的有界挂载需要逐帧预算。
...
HKUST 映射到 `pvs_mainline_v4`。Metropolis 尚未导出 V4 权重，因此使用实例 AABB 视锥模式；每个场景只加载与自身实例表匹配的运行资产。
...
生产构建只携带 `pvs_mainline_v4`。部署脚本会检查运行 schema、文件集合、实例数和场景元数据。
