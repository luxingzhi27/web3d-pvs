...
2. 用预测相机对所有实例 AABB 做 `Frustum.intersectsBox`；
3. 将相交的实例编号压缩为候选数组；
4. 通过 WebGPU 对候选实例执行神经查询；
5. 解码可见性标志、可见性分数和 GLB 下载优先级；
6. 用真实渲染相机再次对模型预测的实例集合做实例 AABB 过滤；
7. 返回 `componentModelList`、GLB 级 immediate/prefetch 列表和调试计时。
...
当前候选 AABB 过滤仍在 Worker 中逐实例执行，并非 GPU 剔除；WebGPU 只负责候选上的神经推理。对大场景而言，候选生成、主线程应用实例矩阵和 GLB 解码可能分别成为瓶颈，需要分开计时。
