...
日期：2026-09-02
状态：已完成分片稀疏化、全场关系归并和 V4 训练 smoke；当前 HKUST 主线与前端资产未改变。
...
- 最终关系 schema 通过现有 V4 训练 preflight 和单步 smoke。

新稀疏关系 CSR 已在小型输入上与稠密构建结果等价，随后删除 IFCBench 的 203 GiB 合并缓存和分片稠密 payload。GPU evidence、渲染日志、manifest 和稀疏正式结果保留。
...
这是数据预处理实现优化，不改变论文三项模型创新、训练损失、阈值协议或前端运行资产。当前默认模型和 HKUST 前端继续使用已登记的 V4 资产。
