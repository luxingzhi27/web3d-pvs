...
新的 `retry1` 回放已完成 2 个场景、3 条轨迹和 2 种兼容 runner/mode 组合。每条轨迹仍使用固定
5 MB/s、50 ms 请求延迟、6 个下载并发、2 个解码/上传并发、冷缓存、`max` GLB 聚合和实际文件字节；
结果没有读取 test，也没有重新选择 visibility 阈值。下表给出三条轨迹的均值和样本标准差。

| 场景 | 方法 | 最终弱效用召回 | 20 MiB 弱效用召回 | 首个有用画面 ms | 下载字节 MB | 无效下载字节 MB |
|---|---|---:|---:|---:|---:|---:|
| HKUST | 当前主线级联 | `0.8749 ± 0.0153` | `0.1982 ± 0.0102` | `133.1 ± 23.3` | `124.7 ± 7.1` | `99.9 ± 7.8` |
| HKUST | 独立 RankNet | `0.8542 ± 0.0175` | `0.1948 ± 0.0199` | `107.9 ± 7.3` | `117.1 ± 6.6` | `110.5 ± 6.8` |
| Metropolis | 当前主线级联 | `0.9059 ± 0.0170` | `0.3004 ± 0.0398` | `78.1 ± 2.4` | `67.7 ± 0.7` | `46.3 ± 4.2` |
| Metropolis | 独立 RankNet | `0.6700 ± 0.0227` | `0.1509 ± 0.0443` | `69.4 ± 13.8` | `75.2 ± 2.4` | `57.4 ± 0.2` |

当前级联在这组离线回放的最终弱效用和 20 MiB 工作点上高于独立 RankNet，尤其在 Metropolis 更明显；
HKUST 的独立 RankNet 首个有用画面更早，但无效下载字节更多，说明单一“首帧更快”不能代表完整轨迹
效用更高。这里的弱效用仍是 `log1p(visible_weights)`，不是像素覆盖率；均值旁的标准差来自三条固定
轨迹，不是三种训练种子或 bootstrap 置信区间。因此 M7/M8 联合调度质量门仍未通过，结果只作为当前
主线的离线调度诊断。

正式产物为：

```text
benchmark/out/m8_formal_current_<scene>_track_<a|b|c>_20260802_retry1.json
benchmark/out/m8_formal_ranknet_<scene>_track_<a|b|c>_20260802_retry1.json
```
