...
def _pose(
    ...
    *,
    ...
    return PoseScores(
        ...
        split=split,
        candidate_ids=np.arange(len(scores), dtype=np.uint32),
        scores=np.asarray(scores, dtype=np.float32),
        targets=np.asarray(targets, dtype=np.uint8),
        visible_weights=np.asarray(weights, dtype=np.float32),
    )
...
class V5MetricsTests(unittest.TestCase):
    ...
    def test_calibration_strict_tier_precedes_higher_mean_only_tier(self) -> None:
        selected = select_calibration_workpoint([
            {
                "threshold": 0.8,
                "selection_split": "calibration",
                "test_read": False,
                "scene_metrics": {"scene": {"weighted_recall": 0.999, "weighted_recall_lcb": 0.989}},
            },
            {
                "threshold": 0.7,
                "selection_split": "calibration",
                "test_read": False,
                "scene_metrics": {"scene": {"weighted_recall": 0.995, "weighted_recall_lcb": 0.991}},
            },
        ...
        selected = select_calibration_workpoint([
            {
                ...
                "selection_split": "calibration",
                "test_read": False,
                "scene_metrics": {"scene": {"weighted_recall": 0.995, "weighted_recall_lcb": 0.989}},
            },
            {
                "threshold": 0.7,
                "selection_split": "calibration",
                "test_read": False,
                "scene_metrics": {"scene": {"weighted_recall": 0.98, "weighted_recall_lcb": 0.97}},
            },
        ], protocol="target_calibrated")
    ...
    def test_test_rows_cannot_enter_threshold_selection(self) -> None:
        with self.assertRaisesRegex(ValueError, "calibration"):
            calibrate_target([_pose(0, [0.9], [1], [1.0], split="test")])

    def test_source_global_uses_only_explicit_source_calibration(self) -> None:
        source = {
            "source_a": [_pose(0, [0.9], [1], [1.0], split="calibration", scene="source_a")],
            "source_b": [_pose(0, [0.9], [1], [1.0], split="calibration", scene="source_b")],
        }
    ...
    def test_exact_calibration_bootstrap_is_logarithmic_in_change_points(self) -> None:
        ...
        for pose_index in range(32):
            scores = np.linspace(pose_index * 1e-5, 1.0 + pose_index * 1e-5, 128)
