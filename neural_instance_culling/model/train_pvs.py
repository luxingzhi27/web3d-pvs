...
from common.visibility_loss import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--utility-loss-weight", type=float, default=0.0)
    parser.add_argument("--download-loss-weight", type=float, default=0.0)
    ...
    parser.add_argument("--boundary-tail-weight", type=float, default=0.0)
    parser.add_argument("--negative-band-weight", type=float, default=0.0)
    ...
    parser.add_argument("--glb-resource-weight", type=float, default=0.0)
    ...
    parser.add_argument("--rvl-count-weight", type=float, default=0.0)
    parser.add_argument(
    ...
    parser.add_argument("--rvl-rank-weight", type=float, default=0.0)
    ...
    parser.add_argument("--schedule-gradient-cap", type=float, default=0.0)
    parser.add_argument("--efficiency-gradient-cap", type=float, default=0.0)
...
def main() -> None:
    ...
    if not all(math.isfinite(float(value)) for value in frontier_scalars):
        raise ValueError("shared-tail parameters must be finite")
    if (
        not 0.0 < args.frontier_positive_mass_fraction <= 1.0
        ...
        raise ValueError("shared-tail parameters are invalid")
    integrated_scalars = (
